...
# 嵌入器类型配置
# 优先读取 DEEPWIKI_EMBEDDER_TYPE，回退到 EMBEDDER_TYPE（docker-compose.yml 中使用），默认 openai
EMBEDDER_TYPE = (os.environ.get('DEEPWIKI_EMBEDDER_TYPE') or os.environ.get('EMBEDDER_TYPE', 'openai')).lower()
...
def load_embedder_config():
    ...
    # 处理各嵌入器的客户端类绑定（local 类型无需 client_class 绑定）
...
def get_embedder_config():
    ...
    if embedder_type == 'local' and 'embedder_local' in configs:
...
# 更新嵌入器配置
if embedder_config:
    for key in ["embedder", "embedder_ollama", "embedder_local", "retriever", "text_splitter"]:
