    ...
    try:
        ...
        if access_token:
            ...
            if repo_type == "github":
                ...
                # Gitea: https://{token}@{domain}/owner/repo.git
                clone_url = urlunparse((parsed.scheme, f"{encoded_token}@{parsed.netloc}", parsed.path, '', '', ''))
            elif repo_type == "gitee":
                # Gitee: https://{token}@gitee.com/owner/repo.git
                clone_url = urlunparse((parsed.scheme, f"{encoded_token}@{parsed.netloc}", parsed.path, '', '', ''))
            else:
                # Generic: try token@host format
                clone_url = urlunparse((parsed.scheme, f"{encoded_token}@{parsed.netloc}", parsed.path, '', '', ''))
...
def prepare_data_pipeline(embedder_type: str = None, is_ollama_embedder: bool = None):
    ...
    if embedder_type == 'ollama':
        ...
        # Use batch processing for OpenAI-compatible embedders
...
def get_gitea_file_content(repo_url: str, file_path: str, access_token: str = None, repo_type: str = "gitea") -> str:
    """
    Retrieves the content of a file from a Gitea or Gitee repository using its API.
    """
    try:
        parsed = urlparse(repo_url)
        ...
        if repo_type == "gitee":
            ...
            # Gitea: use the same host as the repo URL
            api_base = f"{parsed.scheme}://{parsed.netloc}/api/v1/repos/{owner}/{repo}/contents/{file_path}"
        ...
        logger.info(f"Fetching file content from {repo_type} API: {api_base}")
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, dict) and "content" in data:
                ...
                raise ValueError(f"Unexpected response format from {repo_type} API")
        elif response.status_code == 404:
            raise ValueError(f"File not found on {repo_type}.")
        elif response.status_code == 401:
            raise ValueError(f"Unauthorized access to {repo_type}. Check your access token.")
        else:
            response.raise_for_status()
            return response.text
    except RequestException as e:
        raise ValueError(f"Error fetching file content from {repo_type}: {e}")
    except Exception as e:
        raise ValueError(f"Failed to get file content from {repo_type}: {str(e)}")
...
def get_file_content(repo_url: str, file_path: str, repo_type: str = None, access_token: str = None) -> str:
    ...
    if repo_type == "github":
    ...
    elif repo_type in ("gitea", "gitee"):
        return get_gitea_file_content(repo_url, file_path, access_token, repo_type)
    elif repo_type == "local":
        # For local repos, read file directly from disk
        ...
        # Fallback: try reading from cloned repo in adalflow cache
        root_path = get_adalflow_default_root_path()
        url_parts = repo_url.rstrip('/').split('/')
        if len(url_parts) >= 2:
            owner = url_parts[-2]
            repo_name = url_parts[-1].replace(".git", "")
            cached_path = os.path.join(root_path, "repos", f"{owner}_{repo_name}", file_path)
        else:
            repo_name = url_parts[-1].replace(".git", "")
            cached_path = os.path.join(root_path, "repos", repo_name, file_path)
        if os.path.exists(cached_path):
            with open(cached_path, 'r', encoding='utf-8', errors='replace') as f:
                return f.read()
        raise ValueError(f"Unsupported repository type '{repo_type}' or file not found.")
...
class DatabaseManager:
    ...
    def _extract_repo_name_from_url(self, repo_url_or_path: str, repo_type: str) -> str:
        ...
        if repo_type in ["github", "gitlab", "bitbucket", "gitea", "gitee"] and len(url_parts) >= 5:
