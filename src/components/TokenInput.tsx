          ...
          {allowPlatformChange && (
            <div className="mb-3">
              ...
              <div className="flex gap-2 flex-wrap">
                {(['github', 'gitlab', 'bitbucket', 'gitea', 'gitee'] as const).map((platform) => (
                  <button
                    key={platform}
                    type="button"
                    onClick={() => setSelectedPlatform(platform)}
                    className={`flex-1 min-w-[70px] flex items-center justify-center gap-2 px-3 py-2 rounded-md border transition-all ${selectedPlatform === platform
