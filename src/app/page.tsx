...
export default function Home() {
  ...
  const parseRepositoryInput = (input: string): {
    ...
    // Handle Unix/Linux absolute paths (e.g., /path/to/folder)
      ...
      if (lowerInput.includes('/svn/') || lowerInput.includes('svn.')) {
        ...
        type = 'gitea'; // default to gitea for self-hosted git services
      }
  ...
  const handleGenerateWiki = async () => {
    ...
    if (!parsedRepo) {
      setError('Invalid repository format. Use GitHub/GitLab/Gitea/Gitee URL, or a local folder path like "/path/to/folder" or "C:\\path\\to\\folder".');
